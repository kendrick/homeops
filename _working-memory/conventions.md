...
- `recommended` — **Per-execution implementation plans get a copy in the maintainer's Obsidian vault at `30.03 Homelab/_plans/<issue-number>-<short-slug>.md`** (in addition to the local plan at `~/.claude/plans/`). Drop the copy at the start of execution; delete or archive after the issue closes. _Why: meatspace execution often ties up both the Mac (SSH, terminal) and the target hardware (install console, BIOS); the phone via Obsidian iCloud is the most ergonomic surface for following steps. Dedicated `_plans/` folder keeps these ephemeral artifacts findable without polluting `homeops/` (the bidirectional repo mirror) or `_raw/` (input thinking)._
