...
- **Whether `sync-from-vault.sh` should compare content instead of mtime.** `sync-to-vault.sh` guards repo→vault writes with `rsync --update` (added in 3aa9da7) so an in-progress Obsidian edit isn't clobbered. That guard reads mtime as a proxy for newer content, which any formatter defeats. The Obsidian Linter rewrote the vault's `antipatterns.md` in place on 2026-08-21, bumping its mtime past the repo copy while leaving the content a commit behind, so `--update` pinned the stale version permanently until it was force-copied by hand. `sync-from-vault.sh` meanwhile has no guard at all, which leaves the only direction that can overwrite committed work on the unprotected side. Options: compare checksums (`rsync -c`); refuse to pull while `git status` is dirty; or drop the mtime heuristic and make the direction explicit per run. Decide if a second file gets pinned.
